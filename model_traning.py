...
# Using 'viridis' colormap to look different from standard examples
sns.heatmap(
    correlation_df, 
    annot=True, 
    fmt=".2f", 
...
# ---------------------------------------------------------
# 3.1 TEST SET HEATMAP (Specific Requirement)
# ---------------------------------------------------------
...
sns.heatmap(
    ...
    annot=True, 
    fmt=".2f", 
    cmap='magma', # Different color scheme
    linewidths=0.5
)
...
def manage_model_training(algo_name, file_name, clf, x_data, y_data):
    ...
    if os.path.exists(full_path):
        print(f"    [LOAD] {algo_name} loaded from disk.")
...
for name, f_name, obj in classifiers:
    trained_model = manage_model_training(name, f_name, obj, X_trn_sc, y_trn)
